...
void read_weights(char *in_file, int lvls) {
	...
	printf("Reading the weigths of the convolutional layers\n");
	for (z = 0; z < 13; z++) {
	...
	printf("Reading the weigths of the fully connected layers\n");
	for (z = 0; z < 3; z++) {
		if (total_lvls_read >= lvls && lvls != -1)
			break;
...
int main(int argc, char *argv[]) {
	...
	while (!feof(file_list)) {
		...
		normalize_image();
		int i,j,l;
			printf("Image Data (3 channels, 224x224 pixels):\n");
		// dump_image();
		get_VGG16_predict();
		output_predictions(results);
		gettimeofday(&timeEnd, NULL);
		deltaTime = get_seconds(timeStart, timeEnd);
		printf("Sequential time to infer the image %s: %.3lf sec\n", buf, deltaTime);
	}
